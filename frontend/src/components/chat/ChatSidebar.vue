...
<template>
  <aside
    class="grid h-full min-h-0 min-w-0 grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden border-r border-sidebar-border bg-sidebar text-sidebar-foreground max-md:grid-cols-[auto_minmax(0,1fr)] max-md:grid-rows-1 max-md:border-b max-md:border-r-0"
  ...
  >
    <div v-if="collapsed" class="grid content-start justify-items-center gap-2 max-md:flex">
      <Tooltip>
        <TooltipTrigger as-child>
          <Button type="button" variant="ghost" size="icon" aria-label="展开会话栏" @click="emit('toggleCollapse')">
      ...
      <Tooltip>
        <TooltipTrigger as-child>
          <Button type="button" variant="secondary" size="icon" aria-label="新建会话" @click="emit('create')">
    ...
    <div v-else class="grid min-w-0 gap-3 max-md:block">
      <div class="flex items-center justify-between gap-2 max-md:hidden">
        <div class="min-w-0 px-1">
          ...
          <h2 class="m-0 mt-1 text-sm font-semibold">会话</h2>
        </div>
        <Tooltip>
          <TooltipTrigger as-child>
            <Button type="button" variant="ghost" size="icon-sm" aria-label="收起会话栏" @click="emit('toggleCollapse')">
      ...
      <Button variant="secondary" class="w-full justify-start max-md:w-auto max-md:px-4" size="lg" aria-label="新建会话" @click="emit('create')">
        <span>新建会话</span>
      </Button>
    </div>
    ...
    >
      <div
        ...
        :class="session.id === activeSessionId ? 'bg-sidebar-accent text-sidebar-accent-foreground' : 'text-muted-foreground hover:bg-sidebar-accent/60 hover:text-sidebar-accent-foreground'"
      >
        <button
          type="button"
        ...
        >
          <span class="min-w-0 flex-1">
            ...
            <span class="mt-0.5 block text-[11px] text-muted-foreground">{{ formatUpdatedAt(session.updatedAt) }}</span>
          </span>
        </button>
        <Button
          type="button"
      ...
      <div v-if="!sessions.length" class="grid place-items-center px-3 py-10 text-center text-muted-foreground max-md:hidden">
        <p class="m-0 text-xs leading-5">创建第一个会话，开始与模型协作。</p>
      </div>
    </nav>

    <p v-if="!collapsed" class="m-0 border-t border-sidebar-border px-1 pt-3 text-[11px] leading-4 text-muted-foreground max-md:hidden">
      对话内容会自动保存
    </p>
  </aside>
</template>
