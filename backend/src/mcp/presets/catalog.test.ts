...
test('context7 preset has correct structure', () => {
  ...
  assert.equal(context7Preset.version, 1);
  ...
  assert.equal(context7Preset.package.version, '0.1.5');
...
test('memory preset has correct structure', () => {
  ...
  assert.equal(memoryPreset.version, 1);
  ...
  assert.equal(memoryPreset.package.version, '0.1.0');
  assert.equal(memoryPreset.approvalPolicy.default, 'require_approval');
  assert.equal(memoryPreset.approvalPolicy.tools?.['query_memory'], 'allow');
  assert.equal(memoryPreset.approvalPolicy.tools?.['store_memory'], 'require_approval');
});

test('resolveDraft generates correct config for Windows', () => {
  const draft = context7Preset.resolveDraft('win32');
  assert.equal(draft.name, 'Context7 文档');
  assert.equal(draft.transport, 'stdio');
  assert.equal(draft.command, 'npx.cmd');
  assert.deepEqual(draft.args, ['--yes', '@upstash/context7-mcp@0.1.5']);
  assert.equal(draft.authMode, 'none');
  assert.equal(draft.enabled, false);
});

test('resolveDraft generates correct config for macOS', () => {
  const draft = memoryPreset.resolveDraft('darwin');
  assert.equal(draft.name, 'Memory 本地记忆');
  assert.equal(draft.transport, 'stdio');
  assert.equal(draft.command, 'npx');
  assert.deepEqual(draft.args, ['--yes', '@modelcontextprotocol/server-memory@0.1.0']);
  assert.equal(draft.authMode, 'none');
  assert.equal(draft.enabled, false);
});
